...
export class OFXParser {
  private static readonly pattern = /(?<tag>[A-Z0-9./_ ]+?)>((<!\[CDATA\[(?<cdata>.+)\]\]>)|(?<text>[^<]+))?(<\/(?<closetag>\1)>)?/g

  constructor() {

  }

  private getNodeData(match: RegExpMatchArray): { tag: string, text?: string, closetag?: string } {
    ...
    if (cdata !== undefined && text !== undefined) {
      throw Error(`Error parsing ${match.toString()}: both cdata and text were present.`)
    }
  ...
  private closeSGMLTags(ofxString: string, doc: XMLDocument): XMLDocument {
    ...
    for (const match of matches) {
      ...
      if (tag.startsWith('/')) {
        ...
        if (curr !== null && curr?.parentElement !== null && curr?.parentNode !== doc.getRootNode()) {
      ...
      if (text !== undefined && text.length !== 0) {
        ...
        if (curr.parentElement !== null && curr.parentElement !== doc.getRootNode()) {
          curr = curr.parentElement
        }
